...
int spaceshipScore = 5;//used for the life points of the spaceship. Once the spaceship is hit five times, the game is over
...
public void gameOn(){
  ...
  if (!gameOver) { //if the game is not over yet.
     ...
     if(timer.passedTime > 95200){//the timer fucntion. If 95.2 seconds has pass, show the boss,remove all enemies, and start the BOSSMusic.
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 //Gameoverscreen
  ...
  text("Boss HP: ", 300 , 20 );//boss HP tracks the amount of hits needed or hits remaining to destroy the boss.
  text(bossScore, 350, 20);// works almost the same as the "game score" counter but in reverse.
  
  text("HP: ", 600 , 20 );//HP tracks the amount hits remaining until the spaceship is destroyed by the fifth hit.
  text(spaceshipScore, 650, 20);//works exaclty like the "boss HP" counter.
}
...
void updateEnemies() { 
  ...
  if (frameCount%20 == 0) makeEnemy();// every 20 frames make a new target
  ...
  for (int i =enemies.length-1; i>=0; i--) {       
    ...
    for (int j = 0; j<bullets.length; j++) { 
      ...
      if ((bullet.enabled) && (enemy.hitTest(round(bullet.xPos), round(bullet.yPos)))) { 
        ...
        frameRate(15);//this determines how fast the animation runs when an enemy is destroyed.
